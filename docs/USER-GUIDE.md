...
- **`--loader-base-url URL`** (`.env`: `DEIONIZER_LOADER_BASE_URL`) — the base URL
  the ionCube loader bundles are downloaded from when an image is built. This repo
  deliberately does **not** hardcode or redistribute the vendor's loader or its
  distribution endpoint, so you supply it. The build fetches
  `ioncube_loaders_lin_<arch>.zip` from that base, falling back to dated
  `ioncube_loaders_lin_<arch>_<version>.tar.gz` bundles for older PHP. Get the
  current URL from the vendor's downloads page. Required to build any image:

```bash
bin/deionizer build 7.4 --loader-base-url 'https://<loader-download-host>/loader_downloads'
```
