...
the environment — a runner such as `+"`chore`"+` reads your .env and passes these):
  --loader-dir DIR        your own unzipped ionCube loader directory. Required to
                          BUILD a runtime image (the arch+version-matched
                          ioncube_loader_lin_<ver>.so is copied from here); the tool
                          never downloads the vendor's loader. Not needed once images
                          are built. Accepts <arch>/ subdirs or the flat ioncube/ layout
