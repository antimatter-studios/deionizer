...
// image, closing over the loader directory so the fixed EnsureFunc signature
// carries no config of its own — the value still comes from the command line.
func oracleEnsurer(loaderDir string) detect.EnsureFunc {
	return func(ver, _ /*image*/, platform string) error {
		return docker.EnsureOracleImage(ver, platform, loaderDir)
	}
}
...
func resolveVersion(sample string, m *runtime.Matrix, opt options, u *ui) (ver, how string, err error) {
	...
	ver, err = detect.ProbeVersion(sample, m, oracleEnsurer(opt.loaderDir))
	if err != nil {
		return "", "", err
	}
...
func cmdProcess(args []string, opt options) error {
	...
	for _, p := range projects {
		...
		if opt.skeleton {
			...
			count, err = runSkeleton(p, ver, image, platform, opt.loaderDir, u)
		} else {
...
func cmdDecode(args []string, opt options) error {
	...
	if opt.skeleton {
		image := imageOr(rt.Image, docker.ImageName(ver))
		n, err := runSkeleton(dir, ver, image, platform, opt.loaderDir, u)
		if err != nil {
			return err
		}
...
func cmdBuild(args []string, opt options) error {
	...
	if err := docker.EnsureOracleImage(ver, platform, opt.loaderDir); err != nil {
		return err
	}
...
func runSkeleton(dir, ver, image, platform, loaderDir string, u *ui) (int, error) {
